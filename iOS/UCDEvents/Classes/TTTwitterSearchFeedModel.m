...
- (void)load:(TTURLRequestCachePolicy)cachePolicy more:(BOOL)more {
  if (!self.isLoading && TTIsStringWithAnyText(_searchQuery)) {
    ...
    TT_RELEASE_SAFELY(response);
	 		...
	 		NSLog(@"cache age is currently %@", request.cacheExpirationAge); 
	  ...
	  if (request.respondedFromCache) {
		  NSLog(@"Responded from cache with age of %@", request.cacheExpirationAge);
	  } else {
		  NSLog(@"pulled from web ... with cache age of %@", request.cacheExpirationAge);
	  }
  }
}
